;;; spice-mode.el --- Major mode for SPICE -*- lexical-binding: t -*-
...
(spice2g6/spice3/hspice/eldo(rf&verilog-a)/fasthenry/
layla/mondriaan/draccdl/spectre).")
...
(defcustom spice-simulator-switches "" ; example "-noconf"
  "Spice command switches, used when compiling buffer with
`compile-mode', see also `spice-simulator'."
  :group 'spice-simulate
  :type  'string)
...
(defun spice-simulation-buffer-name-function (_arg)
...
(defvar spice-waveform-viewer-read-command t
  "Variable containing boolean indicating reading of buffer
local waveform viewer command.")
...
(defun spice-run-silent (name command _file)
...
(defvar spice-subckt-search-master-filename nil
  "Latest top level (identified by .end in file) .cir file
used in subcircuit searches.")
...
(defun spice-comment-indent-new-line (&optional _soft)
...
;;;###autoload
(add-to-list 'auto-mode-alist (cons "\\.ckt$" 'spice-mode))
